...
const getPoolConfig = () => {
  ...
  const database = process.env.DB_NAME || 'zetechverse';
  const password = String(process.env.DB_PASSWORD ?? '');
  const port = toInt(process.env.DB_PORT, 5432);
  const max = toInt(process.env.DB_POOL_MAX, 20);
  ...
  const keepAlive = String(process.env.DB_KEEP_ALIVE || 'true').toLowerCase() !== 'false';
